...
static bool out_of_bounds(i8 x, i8 y) {
	return x < 0 || y < 0 || x >= WIDTH || y >= HEIGHT;
}

static void tetronimo_spawn(u8 x, u8 y, u8 type, u8 rotation) {
	falling_type = type;
	falling_x = x;
	falling_y = y;
	falling_rotation = rotation;
}
static int tetris_remove_full_row(int y) {
		...
		for (int x = 0; x < TSIZE; x++) {
			...
			if (c > 0) {
				...
				if (!out_of_bounds(posx, posy)) {
					__x = (posx * RECT_WIDTH) + RECT_WIDTH;
					__y = (posy * RECT_HEIGHT);
					__w = (RECT_WIDTH);
					__h = (RECT_HEIGHT);
					GFX_FillRect(__x, __y, __w, __h, c);
				}
...
static void falling_move(int dir) {
	falling_x += dir;
	if (check_collision()) {
		falling_x -= dir;
	}
